(function() {
    ...
    videoPane.$inject = ["$q", "ApiFactory", "PanesFactory", "UtilFactory", "STREAM_FORMAT", "LISTENER_NAMES", "CAMERA_MODES", "$window", "STREAM_TYPES", "ACTIONS", "$timeout", "DEVICE_TYPE", "SocketService", "TYPE_MESSAGES", "SettingFactory"];

    function videoPane($q, ApiFactory, PanesFactory, UtilFactory, streamFormats, listenerNames, cameraModes, $window, streamTypes, actions, $timeout, deviceTypes, SocketService, typeMessages, SettingFactory) {
        return {
            scope: {
                pane: "=videoPane",
                index: "=index"
        ...
        function linkFn(scope, element, attrs) {
            ...
            var droppableOptions = {
                tolerance: 'fit',
                accept: '.camera-tree-item, .header-drag',
            ...
            function disconnectVideoPane() {
                ...
                SocketService.sendPeerToPeer(null, typeMessages.UPDATE_STREAM, object);
            }
...
//                    })
          ...
          function getWebSocketURL(uri) {
            if(window.location.origin.substr(0, 5).toLowerCase() === 'https'){
              var wsProto = 'wss';
              var wsPort = 443;
              uri = wsProto + "://" + window.location.hostname + (wsPort !== 443 ? (':' + wsPort) : '') + "/" + uri.substr(uri.lastIndexOf('/') + 1);
            }
            return uri;
          };
                        ...
                        janus.attach({
                            ...
                            onremotestream: function(stream) {
                                ...
                                remoteVideoEl.bind("playing", function() {
                                    SocketService.sendPeerToPeer({type: typeMessages.IS_PLAYING, paneIdx: scope.index});
